...
export const initCommand = new Command('init')
  ...
  .option(
    '-t, --template <name>',
    t('Use template (web, api, cli)', '使用模板 (web, api, cli)'),
    'default',
  )
  ...
  .action(async (projectName: string, options) => {
    ...
    if (existingConfigs.hasOpenCode || existingConfigs.hasBeads) {
      ...
      const response = await prompts({
        ...
        choices: [
          {
            title: t('Keep it (Merge)', '保留现有配置 (Merge)'),
            value: 'merge',
            description: t(
              'Keep existing files, only add Axon config',
              '保留现有文件，仅添加 Axon 配置',
            ),
          },
          {
            title: t('Backup and recreate (Backup)', '备份并创建新配置 (Backup)'),
            value: 'backup',
            description: t(
              'Backup existing directories to .backup then recreate',
              '备份现有目录为 .backup 后重建',
            ),
          },
    ...
    if (!existsSync(projectPath)) {
      spinner.start(
        t(`Creating project directory ${chalk.cyan(name)}`, `创建项目目录 ${chalk.cyan(name)}`),
      );
    ...
    logger.success(
      t(
        `Project ${chalk.bold(name)} initialization complete!`,
        `项目 ${chalk.bold(name)} 初始化完成！`,
      ),
    );
    logger.blank();
    ...
    console.log(
      `  2. ${chalk.cyan('cat GETTING_STARTED.md')}  - ${t('Read Quick Start guide', '阅读入门指南')}`,
    );
    console.log(
      `  3. ${chalk.cyan('ax spec init')}        - ${t('Define project specification', '定义项目规格')}`,
    );
    console.log(
      `  4. ${chalk.cyan('ax plan')}             - ${t('Generate task graph', '生成任务图')}`,
    );
    logger.blank();
  });
