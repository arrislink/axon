...
export const planCommand = new Command('plan')
  .description(t('Generate task graph from specification', '从规格文档生成任务图'))
  .option('--visualize', t('Visualize in browser', '在浏览器中可视化'))
  .option('--output <path>', t('Custom output path', '自定义输出路径'))
  .option('--model <name>', t('Specify model to use', '指定使用的模型'))
  .option('--dry-run', t('Validate only, do not generate', '只验证，不生成'))
  .action(async (options) => {
    ...
    for (const bead of graph.beads.slice(0, 10)) {
      const deps =
        bead.dependencies.length > 0 ? chalk.dim(` (依赖: ${bead.dependencies.join(', ')})`) : '';
