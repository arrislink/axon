...
export const docsCommand = new Command('docs')
    .description(t('Manage project reference documents', '管理项目参考文档'));
...
docsCommand
    ...
    .action(async (filePath, options) => {
        ...
        try {
            ...
            logger.error(t(`Failed to add document: ${(error as Error).message}`, `添加文档失败: ${(error as Error).message}`));
        }
    });
...
docsCommand
    ...
    .action(async (docId) => {
        ...
        try {
            ...
            logger.error(t(`Failed to summarize: ${(error as Error).message}`, `生成摘要失败: ${(error as Error).message}`));
        }
    });
