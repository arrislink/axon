...
export const configCommand = new Command('config').description(
  t('Manage LLM Provider configuration', '管理 LLM Provider 配置'),
);
...
configCommand
  .command('list')
  .alias('ls')
  .description(t('List all available Providers', '列出所有可用的 Provider'))
...
configCommand
  .command('show')
  .description(t('Show current Axon running mode', '显示当前 Axon 运行模式'))
  .action(() => {
...
configCommand
  .command('failover')
  .description(t('Show failover chain', '显示 Failover 链'))
  .action(() => {
    const omo = new OMOConfigReader();
...
configCommand
  .command('setup')
  .description(t('Configuration wizard', '配置向导'))
  .action(async () => {
    console.log(chalk.bold('\n🚀 Axon 环境配置向导\n'));

    // 1. Check OMO
    const hasOMO = await Bun.$`which omo`
      .quiet()
      .then(() => true)
      .catch(() => false);
    const hasBunxOMO = await Bun.$`bunx oh-my-opencode --version`
      .quiet()
      .then(() => true)
      .catch(() => false);
...
configCommand
  .command('keys')
  .description(t('Quick setup API key (via OMO)', '快速设置 API 密钥 (通过 OMO)'))
  .argument('<provider>', t('Provider (anthropic, openai, etc)', '提供商 (anthropic, openai, etc)'))
