...
export const statusCommand = new Command('status')
    ...
    .action(async (options) => {
        ...
        if (graph) {
            ...
            console.log(`\n💰 ${chalk.bold('成本估算')}:`);
            console.log(
                `   ├─ 预计 tokens: ${graph.metadata.total_estimated_tokens.toLocaleString()}`
            );
            console.log(`   └─ 预计成本: $${graph.metadata.total_cost_usd.toFixed(2)}`);
            ...
            if (pending.length > 0) {
                ...
                // Show next
                if (nextExecutable) {
                    console.log(`  ${chalk.yellow('[⏳]')} ${nextExecutable.id}: ${nextExecutable.title} ${chalk.dim('(下一个)')}`);
                }
