...
export class AnthropicClient {
    ...
    constructor(apiKey: string, defaultConfig: AgentConfig, baseURL: string = 'https://api.anthropic.com/v1') {
        if (!apiKey) {
    ...
    async chat(messages: ChatMessage[], options: ChatOptions = {}): Promise<{
        ...
        try {
            const url = this.baseURL.endsWith('/') ? `${this.baseURL}messages` : `${this.baseURL}/messages`;
