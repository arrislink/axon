...
export class DocumentManager {
    ...
    async add(filePath: string, options?: {
        ...
        try {
            ...
            throw new Error(t(`Failed to extract content: ${(error as Error).message}`, `内容提取失败: ${(error as Error).message}`));
        }
    ...
    async addDirectory(dirPath: string): Promise<Document[]> {
        ...
        for (const file of files) {
            try {
                ...
                logger.warn(t(`Skipping file ${file}: ${(error as Error).message}`, `跳过文件 ${file}: ${(error as Error).message}`));
            }
        }
    ...
    search(query: string): Document[] {
        ...
        return this.library.documents.filter(doc =>
...
`;
    ...
    private async extractImageText(filePath: string): Promise<string> {
        ...
        logger.warn(t('Image text extraction is currently limited. Using OCR if available.', '图片内容提取目前受限，正在尝试使用 OCR...'));
    ...
    private scanDirectory(dirPath: string): string[] {
        ...
        for (const entry of entries) {
            ...
            if (stat.isDirectory()) {
                ...
                if (['.pdf', '.docx', '.md', '.yaml', '.yml', '.txt', '.png', '.jpg', '.jpeg'].includes(ext)) {
                    files.push(fullPath);
                }
            }
        }
