...
export class AxonLLMClient {
  ...
  async chat(messages: LLMMessage[], options?: LLMOptions): Promise<LLMResponse> {
    try {
      ...
      // If CLI mode fails, try direct mode with OMO config
      if (this.mode === 'cli') {
        console.warn(`🧠 Axon: CLI 模式调用失败或响应为空，尝试 Direct OMO 模式...`);
        if (process.env['DEBUG']) console.error(error);

        // Check if OMO config has providers
        ...
        // If no OMO providers, try fallback
        console.warn(`🧠 Axon: Direct 模式无可用 Provider，尝试环境变量回退...`);
        this.mode = 'fallback';
        this.initClient();
        return await this.chat(messages, options);
      }

      // If direct mode fails, try fallback
      if (this.mode === 'direct') {
        console.warn(`🧠 Axon: Direct OMO 模式调用失败，尝试环境变量回退...`);
        if (process.env['DEBUG']) console.error(error);
        this.mode = 'fallback';
        this.initClient();
        return await this.chat(messages, options);
