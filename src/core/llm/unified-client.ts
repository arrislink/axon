...
export class UnifiedLLMClient {
  ...
  private async chatAnthropic(
    ...
    const model = this.cleanModelName(
      options?.model || provider.models?.[0] || 'claude-sonnet-4-20250514',
    );
  ...
  private async chatGoogle(
    ...
    const isAntigravityAuth =
      this.omoConfig.hasAntigravityAuth() && apiKey === this.omoConfig.getAntigravityToken();
