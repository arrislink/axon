...
import { OMOConfigReader } from '../llm/omo-config-reader';
...
export class ConfigPriorityResolver {
    resolve(context: {
        ...
        if (!apiKey) {
            ...
            else if (providerName.includes('google') || providerName.includes('gemini')) apiKey = env['GOOGLE_API_KEY'];
            else if (providerName.includes('deepseek')) apiKey = env['DEEPSEEK_API_KEY'];
        }

        // 4. Other Parameters
        const temperature =
            context.cliOptions['temperature'] !== undefined ? Number(context.cliOptions['temperature']) :
                (projectConfig?.agents?.sisyphus?.temperature ?? 0.7);

        const maxTokens =
            context.cliOptions['maxTokens'] !== undefined ? Number(context.cliOptions['maxTokens']) :
