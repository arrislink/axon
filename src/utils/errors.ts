...
export class SkillsError extends AxonError {
  constructor(message: string, suggestions: string[] = []) {
    super(
      ...
      suggestions.length > 0
        ? suggestions
        : ['检查技能库路径是否正确', '运行 `ax skills list` 查看可用技能'],
    );
  }
}

export class CostLimitError extends AxonError {
  constructor(dailyLimit: number, currentUsage: number, estimated: number) {
    super(`超过每日 token 限制 (${dailyLimit.toLocaleString()})`, 'COST_LIMIT', [
