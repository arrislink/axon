...
describe('Beads Graph', () => {
  ...
  test('topologicalSort orders by dependencies', () => {
    const beads: Bead[] = [createBead('c', ['b']), createBead('a', []), createBead('b', ['a'])];
  ...
  test('hasCycle detects circular dependencies', () => {
    const cyclic: Bead[] = [createBead('a', ['b']), createBead('b', ['a'])];
  ...
  test('getNextExecutable returns null when blocked', () => {
    const beads: Bead[] = [createBead('a', [], 'pending'), createBead('b', ['a'], 'pending')];
